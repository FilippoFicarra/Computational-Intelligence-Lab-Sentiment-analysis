    ...
    def get_embeddings(self, texts: List[str], np_file: str) -> None:
        ...
        with torch.no_grad(), tqdm(total=len(texts), desc='Embedding Progress') as pbar:
            for i in range(0, len(texts), self.batch_size):
                ...
                batch_inputs = self.tokenizer.batch_encode_plus(texts[i:i + self.batch_size], add_special_tokens=True,
                ...
                if 'bert' in self.model_name and 'roberta' not in self.model_name:
                    ...
                    outputs = self.model(batch_input_ids, attention_mask=batch_attention_masks)
                # Extract the embeddings for the [CLS] token of each input
                ...
                if i == 0:
                    ...
                    new_embeddings = np.concatenate((existing_embeddings, torch.cat(embeddings, dim=0).cpu().numpy()),
                                                    axis=0)
